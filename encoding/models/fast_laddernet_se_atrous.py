...
from .LadderNetv66_small_atrous import Decoder,BasicBlock,LadderBlock
...
class LadderNet(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=True, lateral=False,
                 norm_layer=BatchNorm2d,dilated=True, **kwargs):
...
class LadderHead(nn.Module):
    def __init__(self, base_inchannels, base_outchannels,out_channels, norm_layer, se_loss,nclass,up_kwargs):
        ...
        self.decoder = Decoder(planes=base_outchannels,layers=4,strides=self.strides)
        self.ladder = LadderBlock(planes=base_outchannels,layers=4,strides=self.strides)
