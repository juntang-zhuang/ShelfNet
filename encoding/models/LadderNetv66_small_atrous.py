...
class Decoder(nn.Module):
    def __init__(self,planes,layers,strides,kernel=3,block=BasicBlock):
        super().__init__()
        self.planes = planes
        self.layers = layers
        self.kernel = kernel
        ...
        for i in range(0, layers-1):
            if self.strides[-1-i]==1:
                self.up_conv_list.append(nn.Conv2d(planes * 2 ** (layers-1 - i), planes * 2 ** max(0, layers - i - 2), kernel_size=1))
            else:
                self.up_conv_list.append(
                nn.ConvTranspose2d(planes * 2 ** (layers-1 - i), planes * 2 ** max(0, layers - i - 2), kernel_size=3,
                                   stride=self.strides[-1-i], padding=1, output_padding=self.strides[-1-i]-1, bias=True))
...
class LadderBlock(nn.Module):
    ...
    def __init__(self,planes,layers,strides,kernel=3,block=BasicBlock):
        ...
        self.down_conv_list = nn.ModuleList()
        for i in range(0,layers-1):
            self.down_conv_list.append(nn.Conv2d(planes*2**i,planes*2**(i+1),stride=strides[i],kernel_size=kernel,padding=self.padding))
        ...
        for i in range(0, layers-1):
            if strides[-1-i]==1:
                self.up_conv_list.append(nn.Conv2d(planes*2**(layers-i-1), planes*2**max(0,layers-i-2), kernel_size=1))
            else:
                self.up_conv_list.append(nn.ConvTranspose2d(planes*2**(layers-i-1), planes*2**max(0,layers-i-2), kernel_size=3,
                                                        stride=strides[-1-i],padding=1,output_padding=strides[-1-i]-1,bias=True))
            self.up_dense_list.append(block(planes*2**max(0,layers-i-2),planes*2**max(0,layers-i-2)))
